...
async function findTextareaCandidateByLocator(page) {
  ...
  for (let idx = 0; idx < maxToInspect; idx += 1) {
    ...
    const candidate = {
      idx,
      // DO NOT encode "textarea >> nth=idx" here; we'll use idx with .nth() later
      selector: "",
      score,
      visible,
      name,
...
if (best.id) {
  locator = page.locator(`textarea#${cssEscape(best.id)}`);
} else if (best.name) {
  locator = page.locator(`textarea[name="${best.name}"]`);
} else {
